...
void setup()
{
  ...
  spi_data[1] = 200;                  // Number of bytes to read (not including the final CRC)
...
void loop()
{
  ...
  // /* Transmit SPI data */
  // spi_data[0] = NAVX_REG_UPDATE_RATE_HZ;                     // Start register address (high bit clear == read)
  // spi_data[1] = 1;                  // Number of bytes to read (not including the final CRC)
  // spi_data[2] = IMURegisters::getCRC(spi_data,2);   // Calculate CRC 
  // Serial.print("RATE:  ");
  // digitalWrite(SS, LOW);
  // for ( int spi_data_index = 0; spi_data_index < 3; spi_data_index++ ) {
  ...
  // /* Verify CRC and display data */
  // spi_crc = IMURegisters::getCRC(data,1);
  // if ( spi_crc != data[1] ) {
  //     Serial.print("SPI CRC ERROR!  ");
  //     Serial.println(spi_crc);
  // } else {
  //   /* Decode received data to floating-point orientation values */
  //   char rate = (data[0]);   // The cast is needed on arduino

  //   /* Display orientation values */
  //   Serial.print("bin:  ");
  //   Serial.print((uint8_t) rate, BIN);
  //   Serial.print("  dec:  ");
  //   Serial.print((uint8_t) rate, DEC);
  //   Serial.println("");
